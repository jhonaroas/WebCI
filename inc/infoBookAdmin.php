            ...
            <tr>
                ...
                <td>
                    <?php
                        ...
                        $nP=mysql_fetch_array($nameProv);
                        echo $nP['Nombre']; 
                        mysql_free_result($nameProv);
                    ?>
                </td>
            </tr>
                ...
                <p class="text-center">
                    <?php 
                        ...
                        if(mysql_num_rows($checkLoanBook)<=0 && mysql_num_rows($checkLoanBook1)<=0){
                            echo '<button class="btn btn-success btn-update" data-code="'.$codeBook.'" data-url="./controller/SelectDataBook.php"><i class="zmdi zmdi-refresh"></i> &nbsp;&nbsp; Actualizar datos de libro</button>';
                        }else{
                ...
                <?php 
                    $checkLoanBook2=ejecutarSQL::consultar("SELECT * FROM prestamo WHERE CodigoLibro='".$fila['CodigoLibro']."'");
                        if(mysql_num_rows($checkLoanBook2)<=0){
                            echo '<form action="controller/DeleteBook.php" method="post" class="form_SRCB" data-type-form="delete"><input value="'.$fila["CodigoLibro"].'" type="hidden" name="primaryKey"><p class="text-center"><button type="submit" class="btn btn-danger"><i class="zmdi zmdi-delete"></i> &nbsp;&nbsp; Eliminar Libro</button></p></form>';
                        }else{
